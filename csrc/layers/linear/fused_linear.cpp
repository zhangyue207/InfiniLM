...
QKVParallelLinear::forward_split(infinicore::Tensor &input) {
    ...
    if (infinilm::backends::infiniops::should_use(input->device())
        ...
        auto q_out = infinilm::backends::infiniops::linear(
            input, this->weight()->narrow({{0, 0, q_out_size_}}), q_bias_opt);
        auto k_out = infinilm::backends::infiniops::linear(
            input, this->weight()->narrow({{0, q_out_size_, k_out_size_}}), k_bias_opt);
        auto v_out = infinilm::backends::infiniops::linear(
    ...
    auto output =
#ifdef INFINILM_ENABLE_INFINIOPS
        infinilm::backends::infiniops::should_use(input->device())
                && this->get_quantization()->get_quant_scheme() == infinicore::quantization::QuantScheme::NONE
            ? infinilm::backends::infiniops::linear(
                input, this->weight(),
